...
>
    <img 
        src={currentHeroImage} // Apni compressed WebP image ka path yahan rakho
        alt="Remote Work Professional" 
        fetchPriority="high" // 👈 Yeh Next.js ke `priority` ka HTML alternative hai
        loading="eager" // 👈 Make sure lazy load na ho
        className="w-full h-full object-contain object-bottom scale-110 lg:scale-[1.15] origin-bottom" 
    />
</motion.div>
                       )}
                   </div>
...
/>
                                ...
                                <div className="mt-auto pt-3 md:pt-5 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 md:gap-5">
                                    ...
                                    <div className="flex items-center gap-2 md:gap-3 w-full sm:w-auto">
                                        <button aria-label={isSaved ? "Remove from saved jobs" : "Save this job"} onClick={(e) => { e.preventDefault(); toggleSave(job.id); }} className={`p-2 md:p-3 rounded-xl border transition-all ${isSaved ? 'bg-red-50 border-red-200 text-red-500 dark:bg-red-900/20' : 'bg-transparent border-slate-200 text-slate-400 hover:text-red-500'}`}><Heart size={18} className={isSaved ? "fill-current" : ""} /></button>
