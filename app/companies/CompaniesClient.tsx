...
import { searchCompanies } from '@/lib/typesenseCompanies';   // 👈 YE LINE ADD KARO
...
export default function CompaniesClient({
  ...
  const [totalCompanies, setTotalCompanies] = useState(initialTotalCompanies || 0);
  ...
  const fetchCompanies = async (currentPage: number, currentSort: string, isReset = false) => {
    ...
    try {
      const { companies: finalCompanies, found } = await searchCompanies({
        page: currentPage + 1, // Typesense 1-indexed hai, humara state 0-indexed hai
        perPage: LIMIT,
        sortOption: currentSort,
        country: filterCountry,
      });

      if (isReset || currentPage === 0) {
        setCompanies(finalCompanies);
        setTotalCompanies(found);
      } else {
        setCompanies((prev) => [...prev, ...finalCompanies]);
      }

      setHasMore(finalCompanies.length === LIMIT);
    } catch (err) {
      console.error('fetchCompanies typesense error', err);
      if (isReset || currentPage === 0) setCompanies([]);
      setHasMore(false);
    } finally {
      setLoading(false);
    }
...
useEffect(() => {
  if (isFirstRender.current) {
    ...
    return; // Server ne pehle se hi default (a-z, All) data de diya hai — dobara mat maango
  }
  setPage(0);
  setHasMore(true);
  fetchCompanies(0, sortOption, true);
}, [sortOption, filterCountry]); 
                ...
                <div className="relative inline-block text-left z-30">
                  ...
                  {showCountryDropdown && (
                      <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#131b2b] border border-gray-200 dark:border-gray-800 rounded-xl shadow-xl z-50 overflow-hidden">
                         {/* 1. Yeh raha aapka Search Bar (Input) */}
<div className="p-2 border-b border-gray-100 dark:border-gray-800">
    <input 
        ...
        value={countrySearch}
        onChange={(e) => setCountrySearch(e.target.value)} // User jo type karega wo state mein save hoga
    />
</div>

<ul className="max-h-60 overflow-y-auto p-1 custom-scrollbar">
    <li onClick={() => { setFilterCountry('All'); setShowCountryDropdown(false); }} className="p-3 text-sm hover:bg-gray-100 dark:hover:bg-[#1a2333] cursor-pointer rounded-lg text-gray-800 dark:text-gray-200">
      Any Country
    </li>
    
    {/* 2. Aur yeh dekhein FILTER lagaya hua hai! Jo type hoga sirf wahi country bachegi */}
    {COUNTRIES.filter(c => c.name.toLowerCase().includes(countrySearch.toLowerCase())).map((c) => (
        <li 
            key={c.name}
            onClick={() => { setFilterCountry(c.name); setShowCountryDropdown(false); }}
