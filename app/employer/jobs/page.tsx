...
export default function EmployerJobsPage() {
  ...
  async function fetchMyJobs() {
    try {
      ...
      // 2. 🟢 SECURE QUERY: Sirf is user ki jobs nikal lao
      const { data, error } = await supabase
        ...
        .select(`
          ...
          applications ( count )
        `)
        .eq('employer_id', user.id) // 👈 JADOO YAHAN HAI: Sirf apni jobs dekho!
                ...
                <div key={job.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 md:p-6 items-center hover:bg-slate-50 dark:hover:bg-[#151b2e] transition-colors group">
                  ...
                  <div className="col-span-1 md:col-span-2 flex justify-start md:justify-center mt-2 md:mt-0">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                      ...
                      {job.approved ? 'Active' : 'Pending Review'}
                    </span>
                  </div>
                  ...
                  {/* Actions */}
                  <div className="col-span-1 md:col-span-1 flex justify-end gap-2 mt-4 md:mt-0 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link href={`/jobs/${job.id}`} target="_blank" className="p-2 text-slate-400 hover:text-indigo-600 bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors" title="View Public Job">
                      <ExternalLink size={18} />
                    </Link>
                    <button className="p-2 text-slate-400 hover:text-blue-600 bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Edit Job">
                      <Edit size={18} />
                    </button>
