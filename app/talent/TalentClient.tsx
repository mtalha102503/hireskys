...
export default function TalentDirectory({ initialTalents = [] }: { initialTalents?: TalentProfile[] }) {
  const [talents, setTalents] = useState<TalentProfile[]>(initialTalents);
  const [filteredTalents, setFilteredTalents] = useState<TalentProfile[]>(initialTalents);
  const [loading, setLoading] = useState(initialTalents.length === 0);
 ...
 useEffect(() => {
    // 🚀 FIX: Server ne already data de diya hai, sirf fallback (agar server data khaali aaya) mein fetch karo
