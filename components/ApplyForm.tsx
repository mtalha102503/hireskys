...
export default function ApplyForm({ job, onBack }: ApplyFormProps) {
  ...
  const [setJob] = useState<any>(null);
  ...
  if (loading) {
    return <div className="flex flex-col justify-center items-center py-20 gap-4 bg-slate-50 dark:bg-[#0B0F19]"><Loader2 className="animate-spin text-indigo-600 w-10 h-10" /> <p className="text-slate-500 font-bold animate-pulse">Loading Application...</p></div>;
  }

  if (!job) {
    return<div className="flex justify-center items-center py-20 text-2xl font-black text-slate-800 dark:text-white bg-slate-50 dark:bg-[#0B0F19]">Job Not Found 😢</div>;
  }
  ...
  return (
    <div className="h-full w-full bg-slate-50 dark:bg-[#0B0F19] animate-in fade-in duration-500">
      ...
      <div className="py-10 px-4">
        <div className="max-w-4xl mx-auto">
          <button 
            onClick={onBack}
            className="mb-6 flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400 transition-colors"
          >
            <ArrowLeft size={18} />
            Back to Jobs
          </button>
          ...
          <div className="bg-white dark:bg-[#111625] rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
            ...
            {!showForm ? (
              ...
              <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                ...
                <div className="mt-8 pt-6 pb-2 flex justify-center border-t border-slate-100 dark:border-slate-800/50">
                  <a 
                    href={`https://www.hireskys.com?utm_source=embed_widget_apply&utm_medium=powered_by_badge`}
                    target="_blank" 
                    rel="noopener" 
                    className="group inline-flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 transition-all px-4 py-2 rounded-full hover:bg-slate-50 dark:hover:bg-slate-800/50 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 hover:shadow-sm"
                  >
                    <span>Powered by</span>
                    <div className="flex items-center gap-1.5">
                      <img src="/logo2.png" alt="HireSkys" className="w-5 h-5 object-contain group-hover:scale-110 transition-transform" />
