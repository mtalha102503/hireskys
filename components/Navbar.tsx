...
import { 
  ...
  Briefcase, Users, Bell, CheckCircle2, CheckCheck, LayoutDashboard, Wrench, Sparkles // <-- Sparkles add kiya
} from 'lucide-react';
...
useEffect(() => {
  ...
  return (
    <header className="w-full h-16 relative z-50">
      <div className="fixed top-0 left-0 right-0 w-full flex flex-col font-sans z-50">
        {/* 🔵 NAVBAR START */}
        <nav className="w-full border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-[#0B0F19]/90 backdrop-blur-md shadow-sm">
          <div className="container mx-auto px-4 h-16 flex items-center justify-between max-w-7xl lg:px-8">
            ...
            <div className="hidden md:flex items-center gap-4">
              {!user && (
                <Link href="/#jobs" className="text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 px-4 py-2 rounded-xl transition-all duration-200">
                    Jobs
                </Link>
              )}
              <Link href="/talent" className="text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 px-4 py-2 rounded-xl transition-all duration-200">
                  Talent
              </Link>
              {pathname !== '/onboarding' && (
                <Link href="/companies" className="text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 px-4 py-2 rounded-xl transition-all duration-200">
                    Companies
                </Link>
              )}

              <Link href="/tools" className="text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 px-4 py-2 rounded-xl transition-all duration-200">
                  Tools & Perks
              </Link>
              {/* MCP AI Search — Desktop */}
<Link 
  href="/mcp" 
  className="relative flex items-center text-sm font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 border border-indigo-200 dark:border-indigo-700/50 px-4 py-2 rounded-xl transition-all duration-300"
>
  MCP AI Search
  <span className="absolute -top-2 -right-2 bg-gradient-to-r from-emerald-400 to-emerald-500 text-white text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md shadow-sm transform rotate-3">
    New
  </span>
</Link>
              {!user && (
                <Link href="/ats" className="relative flex items-center gap-2 text-sm font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 border border-indigo-200 dark:border-indigo-700/50 px-4 py-2 rounded-xl transition-all duration-300 shadow-sm hover:shadow-md">
              ...
              {user && (
                <Link href="/history" className="text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 px-4 py-2 rounded-xl transition-all duration-200">
                    History
                </Link>
              )}
              {!user && (
                <Link href="/post-job" className="flex items-center gap-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 px-5 py-2.5 rounded-full transition shadow-lg shadow-indigo-500/30 transform hover:-translate-y-0.5">
                          ...
                          <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-[#111625] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-50 overflow-hidden">
                              <div className="p-4 border-b flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                                  ...
                                  {unreadCount > 0 && (
                                      <div className="flex items-center gap-3">
                                          <button onClick={handleMarkAllAsRead} className="flex items-center gap-1 text-[11px] font-bold text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                  ...
                                  {notifications.length === 0 ? (
                                      ...
                                      notifications.map((notif) => (
                                          <div key={notif.id} onClick={() => handleNotificationClick(notif.id, notif.post_id)} className={`p-4 border-b cursor-pointer flex gap-3 ${!notif.is_read ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : ''}`}>
                                              <img src={notif.sender_avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${notif.sender_name?.replace(/\s+/g, '') || 'User'}`} alt="avatar" className="w-8 h-8 rounded-full bg-slate-200 mt-1 object-cover" />
                                              <div>
                              ...
                              {notifications.length > 0 && (
                                  <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 text-center relative z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                                      <button onClick={handleClearAll} className="text-xs font-bold text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400 transition-colors py-1 px-3 rounded-md hover:bg-red-50 dark:hover:bg-red-500/10">
              ...
              {mounted && (
                <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition ml-2">
          ...
          {isOpen && (
            <div className="md:hidden absolute top-16 left-0 w-full bg-white dark:bg-[#0B0F19] border-b border-slate-200 dark:border-slate-800 p-4 shadow-xl animate-fade-in-down z-40">
              <div className="flex flex-col space-y-4">
                {!user && (
                    <Link href="/post-job" onClick={() => setIsOpen(false)} className="flex items-center justify-center gap-2 w-full py-3 bg-indigo-600 text-white font-bold rounded-xl mb-2 shadow-md shadow-indigo-500/20">
                ...
                {/* MCP AI Search — Mobile */}
<Link href="/mcp" onClick={() => setIsOpen(false)} className="flex items-center gap-2 py-2 text-slate-700 dark:text-slate-200 font-medium hover:text-indigo-500">
    AI Search
    <span className="bg-indigo-600 text-white text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md">
      New
    </span>
</Link>
                {!user && (
                ...
                <Link href="/tools" onClick={() => setIsOpen(false)} className="flex items-center gap-2 py-2 text-slate-700 dark:text-slate-200 font-medium hover:text-indigo-500">
                    <Wrench size={18} /> Tools & Perks
                </Link>
